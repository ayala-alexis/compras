...
function crear()
{
	...
	require_once 'view/solc/crear.php';
}
// En controller/solcController.php

function cotizar()
{
	$id_prehsol = isset($_GET['id']) ? (int) $_GET['id'] : 0;
	if ($id_prehsol <= 0) {
		header('Location: ?c=solc&a=consulta_cotizacion');
		exit;
	}

	require_once dirname(__FILE__) . '/../model/model.php';
	$db = new ModelDB();
	$hs = $db->get_solicitud_cabecera($id_prehsol);
	...
	require_once 'view/solc/cotizar.php';
}
...
function consulta_cotizacion()
{
	...
	require_once 'view/solc/consulta_cotizacion.php';
}
function consulta_aprobacion_cc()
{

	require_once 'view/solc/consulta_aprobacion_cc.php';
}
function consulta_pendiente_oc()
{

	require_once 'view/solc/consulta_pendiente_oc.php';
}
function consulta_oc()
{

	require_once 'view/solc/consulta_oc.php';
}
// =====================================================
...
function generar_oc()
{
	...
	$id_prehsol = isset($_REQUEST['id']) ? (int) $_REQUEST['id'] : 0;

	if ($id_prehsol <= 0) {
		die("ID de solicitud no válido.");
	}

	// 2. Extraemos la cabecera ($hs) y el detalle ($ds)
	$hs = $db->get_solicitud_cabecera($id_prehsol);
	$ds = $db->get_solicitud_detalle($id_prehsol);
	...
	// 3. Renderizamos la vista pasándole la información
	require_once 'view/solc/generar_oc.php';
}
...
function revisar_oc()
{
	...
	// 1. Capturamos el ID de la solicitud y el número de OC
	$id_prehsol = isset($_REQUEST['id']) ? (int) $_REQUEST['id'] : 0;
	$numero_oc = isset($_REQUEST['oc']) ? trim($_REQUEST['oc']) : '';

	if ($id_prehsol <= 0 || empty($numero_oc)) {
		die("Parámetros de Orden de Compra no válidos.");
	}

	// 2. Extraemos la información aislada
	$hs = $db->get_oc_cabecera($id_prehsol, $numero_oc);
	$ds = $db->get_oc_detalle($id_prehsol, $numero_oc);
	...
	require_once 'view/solc/revisar_oc.php';
}
// =====================================================
...
function ver_oc()
{
	...
	require_once 'view/solc/ver_oc.php';
}
// =====================================================
...
function revisar_cotizacion_cc()
{
	...
	// 2. Extraemos la cabecera (Esto crea la variable $hs que usa la vista)
	$hs = $db->get_solicitud_cabecera($id_prehsol);

	// 3. Extraemos el detalle con los productos (Esto crea la variable $ds)
	$ds = $db->get_solicitud_detalle($id_prehsol);
	...
	// 4. Renderizamos la vista pasándole la información
	require_once 'view/solc/revisar_cotizacion_cc.php';
}
function crear_copia()
{
			...
			if (mysqli_num_rows($run_sql) <= 0) {
				...
				if ($row[0] > 1) {
					...
					if ($valid_file) {
						...
						$desc_sol = str_replace("'", "''", $desc_sol);
						$id_producto = isset($_POST['predsol_producto']) ? $_POST['predsol_producto'] : "0";
						...
						$sql = "Update " . $conf->getTbl_prehsol()
							...
							. " Where id_prehsol = " . $_POST['id_prehsol'];
						$db->ejecutar($sql);
	...
	$db_sol = new Entity();
	$category = $db_sol->findCategoriasUsuario_v2("Solicitud de compras", 0, 0); //
	$flujo_aproba = $db_sol->flujo_aprobacion_user_compra($infohsol[0]["id_empresa"], $infohsol[0]["id_cc"], 0, $infohsol[0]["id_categoria"]);

	$cat_prod = $db_sol->select_all("select
										id_categoria,
										codigo_categoria,
...
function json_upload()
{
	...
	$campoId = isset($_GET['campo']) ? $_GET['campo'] : '';

	if (empty($campoId) || $id <= 0) {
	...
	if ($archivo['size'] > $maxSize) {
		echo json_encode(['ok' => false, 'msg' => 'El archivo excede el tamaño máximo de 4MB']);
		exit;
	}
	...
	if (function_exists('finfo_open')) {
		...
		// Si no hay función MIME, confiamos en la extensión (menos seguro)
		// Podríamos también usar el tipo enviado por el navegador como último recurso
		$mime = $archivo['type'];
	}

	if ($mime && !in_array($mime, $allowedMimes)) {
		// Puede que algunos archivos .docx sean detectados como zip; permitimos si la extensión es correcta
		// Aceptamos también application/zip y application/octet-stream si la extensión es xlsx o docx?
		// Mejor ser estricto: solo los mimes exactos.
		echo json_encode(['ok' => false, 'msg' => 'Tipo MIME no permitido']);
		exit;
	}
	...
	// Archivo 1
	$new_file_name1 = 'uploads\\' . $_POST['id'] . "-" . basename($archivo['name']);
	$target1 = $currentdir . '\\uploads\\' . $_POST['id'] . "-" . basename($archivo['name']);

	if (move_uploaded_file($archivo['tmp_name'], $target1)) {
	...
	$sql = "Update " . $conf->getTbl_prehsol()
		. " Set "
		. $campoId . " = '" . $db->escape($target1) . "' "
		. " Where id_prehsol = " . $id;
	$db->ejecutar($sql);

}
function json_productos_categoria()
{
	$arr = array();
	if (IS_POST()) {
		$db = new Entity();
		$data = $db->select_all("select
									id_producto id,
									codigo_producto,
									descripcion_producto,
									marca,
									precio_sugerido,
									CONCAT(descripcion_producto,' (',marca,')') descripcion
								from Fproductos
								where id_categoria = " . $_POST['id'] . " and activo = 1
		...
		if ($data != null) {
			...
			$arr = array(
				'exito' => false,
				'msj' => 'Categoria no tiene productos asignados.'
			);
		}
	} else {
		$arr = array(
...
function json_catalogo()
{
	$arr = array();
	if (IS_POST() || isset($_GET['cat'])) {
		$db = new Entity();
		$data = $db->select_all("select
									id_producto id,
									codigo_producto,
	...
	if ($row[0] > 0) {
		...
		if ($_FILES['predsol_coti1']['name']) { // Cotizacion 1
			...
			if (!$_FILES['predsol_coti1']['error']) {
				...
				if ($_FILES['predsol_coti1']['size'] > (5 * 1024 * 1000)) { //can't be larger than 5 MB
					$valid_file = false;
					$valid1 = false;
		...
		if ($_FILES['predsol_coti2']['name']) { // Cotizacion 2
			...
			if (!$_FILES['predsol_coti2']['error']) {
				...
				if ($_FILES['predsol_coti2']['size'] > (5 * 1024 * 1000)) { //can't be larger than 5 MB
					$valid_file = false;
					$valid2 = false;
		...
		if ($_FILES['predsol_coti3']['name']) { // Cotizacion 3
			...
			if (!$_FILES['predsol_coti3']['error']) {
				...
				if ($_FILES['predsol_coti3']['size'] > (5 * 1024 * 1000)) { //can't be larger than 5 MB
					$valid_file = false;
					$valid3 = false;
		...
		if ($valid_file) {
			...
			if ($conta_email <= 0) {
				...
				$sql = "Update " . $conf->getTbl_prehsol()
					. " Set prehsol_estado = 4,"
					. " prehsol_numero_sol = prehsol_numero,"
					//. " prehsol_coti1 = '" . $db->escape($target1) . "',"
					//. " prehsol_coti2 = '" . $db->escape($target2) . "',"
					//. " prehsol_coti3 = '" . $db->escape($target3) . "',"
					. " prehsol_obs1 = '" . $db->escape($_REQUEST['observa_sol']) . "'"
					. " Where id_prehsol = " . $_REQUEST["ps"];
...
function trabajoi()
{
	...
	if ($_POST) {
		...
		if ($_POST['accion'] == 'precio') {
			$p = !empty($_POST['producto']) ? $_POST['producto'] : "";
			$postvalue = unserialize(base64_decode($p));
			$sql = "Update " . $conf->getTbl_predsol() . " Set "
